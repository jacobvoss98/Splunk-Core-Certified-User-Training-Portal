...
# 1. Immediate Safety Escape: Check if database exists
if not all_questions:
    st.error("⚠️ The database is empty. Ensure 'questions.json' is located inside the 'Splunk Exam' folder on GitHub.")
    st.stop()

# 2. Quiz Configuration UI
...
# State keys unique to the selected quiz ID
...
# 3. View Switcher (Completely separate layout paths to eliminate nesting syntax errors)
if current_idx >= len(quiz_questions):
    ...
    st.stop() # Stops execution here so an 'else' block isn't required below

# 4. Active Quiz Interface (No else indentation required)
